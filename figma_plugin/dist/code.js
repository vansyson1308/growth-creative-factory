...
async function loadFontSafe(font, context) {
    try {
...
async function preflightTemplateFonts(template) {
    ...
    for (const tn of textNodes) {
        ...
        try {
            if (tn.fontName !== figma.mixed) {
                const ok = await loadFontSafe(tn.fontName, nodeLabel);
                if (!ok)
...
function getMappedValue(row, nodeName, mapping) {
...
async function setTextIfMapped(tn, value, nodeLabel) {
    try {
        if (tn.fontName !== figma.mixed) {
            const ok = await loadFontSafe(tn.fontName, nodeLabel);
            if (!ok)
        ...
        for (let i = 0; i < len; i++) {
            const font = tn.getRangeFontName(i, Math.min(i + 1, tn.characters.length));
            const ok = await loadFontSafe(font, `${nodeLabel} (range ${i})`);
            if (!ok)
                return;
        }
...
async function handleGenerate(payload) {
    ...
    const template = figma.currentPage.findOne((n) => n.type === "FRAME" && n.name === templateName);
    if (!template) {
    ...
    if (!preflight.ok) {
        postStatus(`⚠️ Preflight font issues on ${preflight.failedNodes.length} node(s): ${preflight.failedNodes.join(", ")}. Continuing with skip-on-failure.`);
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        const cloneTextNodes = clone.findAll((n) => n.type === "TEXT");
...
async function handleExportPngs() {
    const generatedFrames = figma.currentPage.findAll((n) => n.type === "FRAME" && n.name.startsWith(GENERATED_PREFIX));
